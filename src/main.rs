    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.animation_playing && self.frames.len() > 1 {
            ...
            if current_time - self.last_animation_time > 1.0 / self.animation_speed as f64 {
                ...
                ctx.request_repaint(); // Request repaint to update animation
            }
        }
                ...
                .show(ui, |ui| {
                    ...
                    for y in 0..height {
                        for x in 0..width {
                            ...
                            // Handle drawing
                            if (interact_rect.clicked() || interact_rect.dragged())
                                && pointer.primary_down()
                            {
                                if !alt {
                                    // Only push_undo once per drag/click
                                    if !undo_pushed {
                                        self.push_undo();
                                        undo_pushed = true;
                                    }
                            ...
                            // Right click erase
                            if interact_rect.secondary_clicked()
                                || interact_rect.dragged() && pointer.secondary_down()
                            {
